                                  ...
                                  ecma_value_t argument) /**< argument for the reaction */
{
  ...
  job_p = JMEM_POOLS_ALLOC (ecma_job_promise_reaction_t);
...
ecma_free_promise_reaction_job (ecma_job_promise_reaction_t *job_p) /**< points to the PromiseReactionJob */
{
  ...
  JMEM_POOLS_FREE (job_p, ecma_job_promise_reaction_t);
                                          ...
                                          ecma_value_t then) /**< 'then' function */
{
  ...
  job_p = JMEM_POOLS_ALLOC (ecma_job_promise_resolve_thenable_t);
                                                                                     ...
                                                                                     *   PromiseResolveThenableJob */
{
  ...
  JMEM_POOLS_FREE (job_p, ecma_job_promise_resolve_thenable_t);
...
ecma_enqueue_job (ecma_job_handler_t handler, /**< the handler for the job */
                  void *job_p) /**< the job */
{
  ecma_job_queueitem_t *item_p = JMEM_POOLS_ALLOC (ecma_job_queueitem_t);
  item_p->job_p = job_p;
...
ecma_process_all_enqueued_jobs (void)
{
  ...
  while (JERRY_CONTEXT (job_queue_head_p) != NULL && !ECMA_IS_VALUE_ERROR (ret))
  {
    ...
    JMEM_POOLS_FREE (item_p, ecma_job_queueitem_t);
